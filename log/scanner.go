...
func (s *Scanner) Scan() (tok Token, lit string) {
	...
	case '\n':
		return TokenNewLine, "\n"
	case '@':
		return s.scanType(TokenFlag, isAlphaNum)
	case '=':
		return s.scanType(TokenValue, isAlphaNum)
	case ',':
		return TokenComma, ","
	case '{':
		return TokenLBrace, "{"
	...
	case '_':
		return s.scanType(TokenIdentifier, isLetter)
	case '>':
		return s.scanType(TokenSent, isLine)
	case '<':
		return s.scanType(TokenRecv, isLine)
	case eof:
		return TokenEOF, ""
	}
